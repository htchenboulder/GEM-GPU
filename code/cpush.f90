...
!$acc update host(w3,x2,x3,y2,y3,z2,z3,u2,u3,w2)
  ...
  !!!!test x2 in host has been changed or not:: x2 has been changed by pmove
  !!if(MyId==0)write(*,*) x2-x2_tmp

  !!!test x2 in device is same with host or not
  !x2_tmp=x2
  !$acc update host(x2)
  !if(MyId==0)write(*,*) x2-x2_tmp
